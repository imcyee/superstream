...
export class BaseListsStorage {
  ...
  key_format = (key, list) => `list:${key}:${list}`
  ...
  constructor(key, kwargs) {
    this.base_key = key
    this.key_format = kwargs['key_format'] || this.key_format
    this.max_length = kwargs['max_length'] || this.max_length
  ...
  get_key(list_name) {
    ...
    // '''
    return this.key_format(this.base_key, list_name)
  }
