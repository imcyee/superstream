...
export class RedisListCache extends BaseRedisListCache{
    ...
    get_results(start, stop){
       ...
       const  results = this.redis.lrange(key, start, stop)
        return results
}
    append(value){
      const   values = [value]
    ...
    append_many(values){
      const  key = this.get_key()
       ...
       function _append_many(redis, values){
            for( value in values){
                logger.debug('adding to %s with value %s', key, value)
                const   result = redis.rpush(key, value)
                results.append(result)}
            return results
            }
        // # start a new map redis or go with the given one
        results = this._pipeline_if_needed(_append_many, values)

        return results
}
    remove(value){
      const   values = [value]
      const   results = this.remove_many(values)
      const   result = results[0]
        return result
}
    remove_many(values){
      ...
      function  _remove_many(redis, values){
            for (value in values){
                ...
                const  result = redis.lrem(key, 10, value)
                results.append(result)}
            return results
            }
        // # start a new map redis or go with the given one
...
export class FallbackRedisListCache extends RedisListCache{
    ...
    get_results(start, stop){
        ...
        var results
        if (stop is not null){
          const    redis_results = this.get_redis_results(start, stop - 1)
          const    required_items = stop - start
          var    enough_results = len(redis_results) == required_items
            ...
            const    redis_results = this.get_redis_results(start, stop)
            enough_results = true}

        if (! redis_results || ! enough_results){
            this.source = 'fallback'
            const  filtered = getattr("_filtered", false)
            const  db_results = this.get_fallback_results(start, stop)

            if( start == 0 && not redis_results && not filtered){
            ...
            }else if( start == 0 && redis_results && not filtered){
    ...
    get_redis_results(start, stop){
        ...
        // :param stop: the end
        // '''
        const  results = RedisListCache.get_results(start, stop)
        return results
}
