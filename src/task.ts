...
import { getRedisAddress, getRedisConnection } from "."

const redis = getRedisConnection()
const redisAddress = getRedisAddress()
const fanoutQueue = new Queue('fanout_operation', redisAddress);
const fanoutHighPriorityQueue = new Queue('fanoutOperationHiPriority', redisAddress);
const fanoutOperationLowPriorityQueue = new Queue('fanoutOperationLowPriority', redisAddress);
const followManyQueue = new Queue('followMany', redisAddress);
const unfollowManyQueue = new Queue('unfollowMany', redisAddress);
 ...
 * Potential candidate "bull"
 */

fanoutQueue.process((job, done) => {
  const { feedManager, FeedClass, user_ids, operation, operation_kwargs } = job.data;

  // Simple task wrapper for _fanout task
  // Just making sure code is where you expect it :)
  (feedManager as Manager).fanout(user_ids, FeedClass, operation, operation_kwargs)

  const format = (a, b, c, d) => { return `${a} user_ids, ${b}, ${c} ${d}` }
  return format(user_ids.length, FeedClass, operation, operation_kwargs)
})

fanoutHighPriorityQueue.process((job, done) => {
  const { feedManager, FeedClass, user_ids, operation, operation_kwargs } = job.data;
  fanoutQueue.add({
    feedManager,
    FeedClass,
...
fanoutOperationLowPriorityQueue.process((job, done) => {
  const { feedManager, FeedClass, user_ids, operation, operation_kwargs } = job.data;
  fanoutQueue.add({
    feedManager,
    FeedClass,
...
followManyQueue.process(async (job, done) => {
  const { feedManager, user_id, target_ids, follow_limit } = job.data
  const feeds = Object.values((feedManager as Manager).getFeeds(user_id))
 ...
 * @param source_ids 
 */
unfollowManyQueue.process(async (job, done) => {
  const { feedManager, user_id, source_ids } = job.data
