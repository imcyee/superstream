...
// @shared_task
export function fanout_operation(feed_manager, FeedClass, user_ids, operation, operation_kwargs) {
  // '''
...
// @shared_task
export function fanout_operation_low_priority(feed_manager, FeedClass, user_ids, operation, operation_kwargs) {
  return fanout_operation(feed_manager, FeedClass, user_ids, operation, operation_kwargs)
}

// @shared_task
export function follow_many(feed_manager, user_id, target_ids, follow_limit) {
  const feeds = feed_manager.get_feeds(user_id).values()
  ...
  if (activities) {
    for (const feed of feeds) {
      const batch_interface = feed.get_timeline_batch_interface()
...
export function unfollow_many(feed_manager, user_id, source_ids) {
  for (const feed of feed_manager.get_feeds(user_id).values()) {
    const activities = []
    feed.trim()
